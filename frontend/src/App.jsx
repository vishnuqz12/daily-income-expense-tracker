...
const periodLabel = (period) =>
  period
    ? `${dateLabel(period.startDate)} – ${dateLabel(period.endDate || periodEnd(period.startDate))}`
    : "No month started";
    ...
    <div className="dashboard-wrap">
      <section className="dashboard-hero panel">
        <div>
          ...
          <p>
            Comparison uses your manually started salary-cycle months. Nothing
            resets on the 1st unless you start a new month.
          </p>
        </div>
        <div className="dashboard-total">
          <span>NET TOTAL SAVINGS</span>
            ...
            <div className="comparison-card panel" key={label}>
              ...
              <div className="delta">
                {previous
                  ...
                  : "Start a second month to compare"}
              </div>
            </div>
          );
        })}
      ...
      <section className="panel chart-panel">
        <div className="section-title">
          <div>
            <h3>{current ? periodLabel(current) : "No month started"}</h3>
        ...
        {current ? (
          ...
          <div className="empty">
            <strong>Start your first financial month</strong>
            <span>Use Start New Month from the Accounts tab.</span>
          </div>
        )}
      </section>
    </div>
...
function App() {
  ...
  const activePeriod =
    sortedPeriods.find((p) => p.id === periodId) || sortedPeriods[0];
  ...
  useEffect(() => {
    if (activePeriod)
      setTransferForm((f) => ({
        ...f,
        date: isDateWithinPeriod(f.date, activePeriod)
          ? f.date
  ...
  async function saveCurrent() {
    ...
    try {
      await exportWorkbook(data, makeWorkbookFileName(data.profile.email));
      setNotice(
        "Excel backup exported. IndexedDB remains the live data store.",
      );
    } catch (err) {
  ...
  const persist = async (
    next,
    message = "Changes saved to IndexedDB on this device.",
  ) => {
    try {
  ...
  async function addBank(e) {
    ...
    if (!name) return setError("Enter a bank name.");
    if (data.banks.some((b) => b.name.toLowerCase() === name.toLowerCase()))
      return setError("That bank already exists.");
    const bank = { id: uid("bank"), name, createdAt: new Date().toISOString() };
    const next = { ...data, banks: [...data.banks, bank] };
    setBankName("");
    ...
    setTransferForm((f) => ({
      ...f,
      fromBankId: bank.id,
      toBankId: data.banks[0]?.id || "",
    }));
    await persist(next, "Bank added and saved locally.");
  }
  ...
  async function removeBank() {
    ...
    if (
      !window.confirm(
        `Delete ${activeBank.name} and its related transactions and transfers?`,
      )
    )
      return;
    const next = {
      ...
      transfers: data.transfers.filter(
        (t) => t.fromBankId !== activeBank.id && t.toBankId !== activeBank.id,
      ),
    };
    setBankId(next.banks[0]?.id || "");
    await persist(next, "Bank deleted and saved locally.");
  }
  ...
  async function addPeriod(e) {
    ...
    if (data.periods.some((p) => p.startDate === startDate))
      return setError("A financial month with this start date already exists.");
    const period = {
      id: uid("period"),
      startDate,
      endDate: periodEnd(startDate),
      createdAt: new Date().toISOString(),
    };
    const next = { ...data, periods: [...data.periods, period] };
    setNewStartDate(today());
    setShowPeriodForm(false);
    setPeriodId(period.id);
    setForm(makeInitialForm(type, period));
    setTransferForm((f) => ({ ...f, date: period.startDate }));
    await persist(next, "Financial month started and saved locally.");
  }
  ...
  async function addTransaction(e) {
    ...
    if (!activePeriod)
      return setError("Start a financial month before adding transactions.");
    const amount = Number(form.amount);
    if (!(amount > 0)) return setError("Enter an amount greater than 0.");
    if (!isDateWithinPeriod(form.date, activePeriod))
      return setError(
        `Date must be between ${dateLabel(activePeriod.startDate)} and ${dateLabel(activePeriod.endDate || periodEnd(activePeriod.startDate))}.`,
      );
    const transaction = {
      id: uid("txn"),
    ...
    setForm(makeInitialForm(type, activePeriod));
    ...
    () =>
      data.transfers.filter(
        (t) =>
          ...
          (t.fromBankId === activeBank?.id || t.toBankId === activeBank?.id),
      ),
    [data.transfers, activePeriod?.id, activeBank?.id],
  );
  ...
  const summary = useMemo(() => {
    ...
    const transferBalance = data.transfers.reduce(
      (s, t) =>
        s +
        (t.toBankId === activeBank.id ? t.amount : 0) -
        (t.fromBankId === activeBank.id ? t.amount : 0),
      0,
    );
    return {
      income,
  ...
  const transferOptions = sortedBanks.filter(
    (b) => b.id !== transferForm.fromBankId,
  );
  const totalSavings = data.transactions.reduce(
    (sum, t) => sum + (t.type === "income" ? t.amount : -t.amount),
    0,
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div>
          <div className="eyebrow">PERSONAL FINANCE • LOCAL FIRST</div>
          <h1>Daily Money Tracker</h1>
          {/* <p>{data.profile.email} • Stored in this device browser</p> */}
        </div>
      ...
      {tab === "dashboard" ? (
        ...
        <>
          <section className="panel bank-section">
            <div className="section-title">
              <div>
                <h2>Bank Accounts</h2>
                <p>
                  Each bank is a separate tab. Transfers between your own banks
            ...
            {showBankForm && (
              <form className="inline-form" onSubmit={addBank}>
                <input
                  value={bankName}
                  ...
                  autoFocus
                />
                <button className="primary-button small">Add Bank</button>
              </form>
            )}
            {sortedBanks.length ? (
              ...
              <div className="empty compact">
                <strong>Add your first bank account</strong>
                <span>Example: ICICI Bank, HDFC Bank, SBI.</span>
              </div>
            )}
          </section>
          ...
          <section className="panel month-control-panel">
            <div className="section-title">
              <div>
                <h2>Financial Month</h2>
                <p>
                  Start a month only when you choose the date. Your salary cycle
                  is not reset on the 1st.
                </p>
              </div>
              <button
                className="secondary-button"
            ...
            {showPeriodForm && (
              <form className="start-month-form" onSubmit={addPeriod}>
                <label>
                  Start date
                  <input
                    type="date"
                ...
                <div className="month-form-help">
                  Example: 25th → 24th is a natural salary-cycle month.
                </div>
                <button className="primary-button small">Start Month</button>
              </form>
            )}
            {sortedPeriods.length ? (
              ...
              <div className="empty compact">
                <strong>No financial month started</strong>
                <span>
                  Start the first month before adding income, expenses or
                  transfers.
                </span>
              </div>
            )}
          ...
          {activeBank && activePeriod ? (
            <>
              <div className="account-header">
                <div>
                  ...
                  <p>{periodLabel(activePeriod)}</p>
                </div>
                <div className="account-actions">
                  <div className="mini-stat">
                    Selected-month savings{" "}
                    <strong>{money(summary.savings)}</strong>
                  </div>
                  <div className="mini-stat">
                    Bank balance <strong>{money(summary.balance)}</strong>
                  </div>
                  <button className="danger-link" onClick={removeBank}>
                    Delete account
                  </button>
                </div>
              </div>
              ...
              <section className="panel transfer-panel">
                <div className="section-title">
                  <div>
                    <h2>Self Transfer</h2>
                    <p>
                      Move your own money from one bank to another. It affects
                      balances, not income/expense totals.
                    </p>
                  </div>
                ...
                {sortedBanks.length < 2 ? (
                  <div className="empty compact">
                    <strong>Add another bank to enable transfers</strong>
                    <span>
                      The new bank will automatically appear in the transfer
                      list.
                    </span>
                  </div>
                ) : (
                  <>
                    {showTransferForm && (
                      <form className="transfer-form" onSubmit={addTransfer}>
                        ...
                        <button className="primary-button transfer-button">
                          Transfer Money
                        </button>
                      </form>
                    )}
                    {selectedTransfers.length ? (
                      <div className="transfer-list">
                        {selectedTransfers.map((item) => {
                          const from =
                            sortedBanks.find((b) => b.id === item.fromBankId)
                              ?.name || "Unknown";
                          const to =
                            sortedBanks.find((b) => b.id === item.toBankId)
                              ?.name || "Unknown";
                          return (
                      ...
                      !showTransferForm && (
                        <div className="empty compact">
                          <strong>No transfers in this financial month</strong>
                          <span>Your self-transfers will appear here.</span>
                        </div>
                      )
                    )}
                  </>
              ...
              <section className="panel monthly-panel">
                <div className="section-title">
                  <div>
                    ...
                    <p>
                      Income, expense and savings for this bank across your
                      started months.
                    </p>
                  </div>
                  <select
                ...
                {visibleMonths.length ? (
                  <div className="month-list">
                    {visibleMonths.map((m) => (
                      <button
                        key={m.id}
                        className={`month-row ${m.id === activePeriod.id ? "selected" : ""}`}
                        onClick={() => setPeriodId(m.id)}
                      >
                        <span>{periodLabel(m)}</span>
                        <span>Income {money(m.income)}</span>
                        <span className="month-expense">
                          Expense {money(m.expense)}
                        </span>
                        <span
                          className={
                            m.savings >= 0 ? "month-savings" : "month-loss"
                          }
                        >
                  ...
                  <div className="empty compact">
                    <strong>No monthly records</strong>
                  </div>
                )}
                {monthly.length > 6 && (
                  <button
                  ...
                  >
                    {showAllMonths ? "Show recent months" : "View all months"}
                  </button>
                )}
              </section>
                ...
                <section className="panel">
                  <div className="panel-heading">
                    <div>
                      ...
                      <p>
                        {activeBank.name} • {periodLabel(activePeriod)}
                      </p>
                    </div>
                    <div className="type-switch">
                      <button
                        ...
                        onClick={() => {
                          setType("expense");
                          setForm(makeInitialForm("expense", activePeriod));
                        }}
                      >
                        + Expense
                      </button>
                      <button
                        ...
                        onClick={() => {
                          setType("income");
                          setForm(makeInitialForm("income", activePeriod));
                        }}
                      >
                        + Income
                      </button>
                    ...
                    <label>
                      ...
                      <input
                        ...
                        max={
                          ...
                          periodEnd(activePeriod.startDate)
                        }
                        onChange={(e) =>
                ...
                <section className="panel">
                  <div className="panel-heading">
                    <div>
                      <h2>Transactions</h2>
                      <p>
                        Only the selected bank and financial month are shown.
                      </p>
                    </div>
                    <input
                  ...
                  {selectedBankTransactions.length ? (
                    ...
                    <div className="empty">
                      ...
                      <span>
                        Add an income or expense for this financial month.
                      </span>
                    </div>
                  )}
                </section>
            ...
            <section className="panel empty">
              <strong>Select a bank and start a financial month</strong>
              <span>
                Your existing bank tabs and monthly controls stay available
                above.
              </span>
            </section>
          )}
      ...
      {/* <footer className="app-footer">
        Local-first financial tracker • IndexedDB is the source of truth •{" "}
        <button onClick={saveCurrent}>Export Excel backup</button>
      </footer> */}
    </div>
  );
}
